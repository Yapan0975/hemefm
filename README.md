...
2. **Within-cohort comparisons are inconclusive and subject to model-selection bias** — on BeatAML 2.0 (n = 86 val), HemeFM (QWK 0.742) is point-estimate-favoured but inconclusive relative to both a fair-encoder_lr no-pretraining baseline (best QWK 0.571) and a PCA-64 + logistic-regression baseline (QWK 0.787). These within-cohort comparisons are reported as **point estimates only**: no parametric confidence interval or p-value is given, because the same partition was used for hyperparameter/epoch selection and because the Fleiss–Cohen weighted-κ variance and the independent-SE combination of same-patient predictions are not valid (EV6). The only inferential comparison in the study is the external paired bootstrap (point 5).
